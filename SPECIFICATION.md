...
DS-LZ (Delta-Stride LZ) is a domain-optimized LZ77 variant designed for tile-based graphics compression, particularly retro emulator save states. It achieves 822× compression on MSX VRAM by exploiting vertical tile-row correlation through a delta-XOR stride preprocessing step.
...
Pass 2 (horizontal): For i = size-1 down to 1:
...
```
Pass 1 (horizontal): For i = 1 to size-1:
                       If i % stride != 0: data[i] ^= data[i - 1]
...
| **Compression ratio (MSX VRAM title screen)** | 822× |
| **Compression ratio (MSX VRAM in-game)** | 108× |
| **Encoder RAM** | 32 KB (hash table) |
| **Decoder RAM** | 0 bytes (in-place) |
| **Encoder speed (ESP32-S3)** | ~15 MB/s |
| **Decoder speed (ESP32-S3)** | ~200 MB/s |
...
- The decompressor trusts the input stream. Malformed data may cause the decoder to read past the input buffer or write past the output buffer.
...
1. Lempel, A., Ziv, J. (1977). "A Universal Algorithm for Sequential Data Compression." *IEEE Transactions on Information Theory*
2. Storer, J., Szymanski, T. (1982). "Data Compression via Textual Substitution." *Journal of the ACM*
3. Fayzullin, M. (1994-2021). fMSX: Portable MSX Emulator
4. Collet, Y. (2011). LZ4: Extremely Fast Compression
5. Marty, E. (2019). LZSA: Efficient LZ77 Compression for 8-bit CPUs
